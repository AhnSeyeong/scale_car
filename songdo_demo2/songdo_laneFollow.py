...
class DetectLine:
    def __init__(self):
        ...
        self.roi_y_l = rospy.get_param('~roi_y_l', 310)
        ...
        self.white_lane_high = np.array([180, 35, 250])    #180, 45, 255
        ...
        self.min_distance_threshold = rospy.get_param('~min_distance_threshold', 100)  # Minimum distance threshold between contour points
        self.line_distance_threshold = rospy.get_param('~line_distance_threshold', 150)  # Distance threshold between two closest lines
        ...
        self.dst_points = np.float32([
            [self.roi_x_l  , self.roi_y_l],     # Top-left corner
            [self.roi_x_h  , self.roi_y_l],     # Top-right corner
            [self.roi_x_l + 225, self.roi_y_h], # Bottom-left corner
            [self.roi_x_h - 225, self.roi_y_h]  # Bottom-right corner
        ])
    ...
    def calculate_guiding_position(self, guide_center, gear_flag):
        dy = guide_center - 320
        ...
        if gear_flag == 0:
            ...
            msg.drive.speed = 0.45
        
        self.ack_pub.publish(msg)

    def run(self, gear_flag):
        ...
        if len(left_lines) + len(right_lines) >= 2:
            ...
            if len(left_lines) > 0 :
                guide_center.append(left_lines[0] + 150)
                idx_counter += 1

            if len(right_lines) > 0 :
                guide_center.append(right_lines[0] - 150)
                idx_counter += 1
        ...
        if idx_counter != 0:
            ...
            self.calculate_guiding_position(self.current_center, gear_flag)
        else:
            self.calculate_guiding_position(self.current_center, gear_flag)
