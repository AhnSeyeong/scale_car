...
class LaneFollow:
    def __init__(self):
        ...
        # 차선 검출 파라미터
        self.yellow_lane_low = np.array([20, 150, 140])  # Saturation 낮추고, Value 상향
        self.yellow_lane_high = np.array([35, 255, 220]) # Hue 범위 확대
        self.brightness_threshold = 180  # 반사광 제거를 위한 밝기 임계값
        self.min_contour_area = 300     # 최소 컨투어 영역
        self.max_contour_area = 8000    # 최대 컨투어 영역
        ...
        self.dst_points = np.float32([
            [self.roi_x_l, self.roi_y_l],
            [self.roi_x_h, self.roi_y_l],
            [self.roi_x_l + 205, self.roi_y_h],
            [self.roi_x_h - 205, self.roi_y_h]
        ])
    ...
    def preprocess_image(self, image):
        ...
        # 적응형 이진화로 반사광 영역 검출
        v_thresh = cv2.adaptiveThreshold(
            v,
            255,
        ...
        # 반사광 영역 마스크 생성
        reflection_mask = v > 200  # 매우 밝은 영역을 반사광으로 간주
        
        # 반사광 영역의 채도(S) 높이기 (노란색 차선은 채도가 높음)
        s[reflection_mask] = s[reflection_mask] * 1.5
        s = np.clip(s, 0, 255).astype(np.uint8)
        
        # 반사광 영역의 밝기(V) 감소
        v[reflection_mask] = v[reflection_mask] * 0.5
    ...
    def filter_contours(self, contours):
        ...
        for contour in contours:
            ...
            if mean_v > self.brightness_threshold and mean_s < 100:
                continue
    ...
    def calculate_guiding_position(self, guide_center, deaccel_flag):
        ...
        if self.prev_centers:
            ...
            if abs(guide_center - prev_avg) > max_center_change:
                ...
                theta_rad = np.arctan2(guide_center - 320, dx)
        ...
        msg.drive.speed = 0.2 if deaccel_flag else 0.35
    ...
    def run(self, deaccel_flag=1):
        ...
        self.mask_white = cv2.inRange(hsv, self.yellow_lane_low, self.yellow_lane_high)
        ...
        if len(left_lines) + len(right_lines) >= 2:
            ...
            guide_center = left_lines[0] + 155
        elif len(right_lines) == 1:
            guide_center = right_lines[0] - 155
            
            
        if len(left_lines) + len(right_lines) == 0:
            # 이전 프레임의 중심점 유지하되 약간의 보정
            if abs(self.current_center - 320) > 50:  # 중앙에서 많이 벗어난 경우
                self.current_center = int(0.8 * self.current_center + 0.2 * 320)  # 중앙으로 천천히 복귀
...
if __name__ == '__main__':
    try:
        ...
        rospy.sleep(2)
        if lane_follow.image_ is None:
            rospy.logwarn("No image received after 2 seconds!")
        
        rate = rospy.Rate(10)
        while not rospy.is_shutdown():
            if lane_follow.image_ is not None:
                lane_follow.run()
            else:
